...
private let aBerlinCoordinate = Restaurant.Coordinate(lat: 52.507_673, lon: 13.390_335)
private let aYelpKey = "<set your proper api key here>"

class RestaurantsViewModel {
    
    let currentLocation = Variable(aBerlinCoordinate)
...
// MARK: data handling
    ...
    static func request(coordinates: Restaurant.Coordinate) -> URLRequest? {
        ...
        guard aYelpKey.first != "<" else {
            fatalError("Set proper Yelp Api Key!")
        }
    
        request.addValue("Bearer \(aYelpKey)", forHTTPHeaderField: "Authorization")
